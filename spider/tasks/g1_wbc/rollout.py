...
def run_direct_rollout(
    ...
    *,
    ...
    """Roll out by directly applying each qpos frame as a position target (no policy).

    This is the retarget baseline: the reference trajectory directly controls the
    simulator without RL policy intervention.
    """
    device = torch.device(config.device)
    qpos_trajectory = qpos_trajectory.to(device)
    if qpos_trajectory.ndim == 2:
        qpos_trajectory = qpos_trajectory[:, None, :]  # (T, 1, 36)

    total_steps = qpos_trajectory.shape[0]
    ...
    body_pos_trace, body_quat_trace = [], []
    body_lin_vel_trace, body_ang_vel_trace = [], []
    controls = []
    ...
    _append_state(state, foot_contact, foot_force, qpos_trace, qvel_trace,
                  body_pos_trace, body_quat_trace, body_lin_vel_trace,
                  body_ang_vel_trace, contact_indicator, contact_force)
    ref_indices.append(torch.zeros(config.num_envs, dtype=torch.long, device=device))

    zero_action = torch.zeros(config.num_envs, ACTION_DIM, device=device)
    with torch.inference_mode():
        for step_idx in range(total_steps):
            # Use the qpos trajectory directly as position targets
            target_qpos = qpos_trajectory[step_idx]  # (N, 36)
            ctrl = target_qpos[:, 7:]  # joint positions only
            ...
            _append_state(state, foot_contact, foot_force, qpos_trace, qvel_trace,
                          body_pos_trace, body_quat_trace, body_lin_vel_trace,
                          body_ang_vel_trace, contact_indicator, contact_force)
            actions.append(zero_action.detach().clone())
            controls.append(ctrl.detach().clone())
            ref_indices.append(torch.full((config.num_envs,), step_idx,
